...
def plot_dsp(length, baseline, optimized):
    plt.figure()
    ax = plt.gca()
    ax.scatter(np.arange(len(length)), baseline, marker="o", s=75, color="#cccccc")
    ax.scatter(np.arange(len(length)), optimized, marker="^", s=75, color="#666666")
...
def plot_lut(length, baseline, optimized):
    plt.figure()
    ax = plt.gca()
    ax.scatter(np.arange(len(length)), baseline, marker="o", s=100, color="#cccccc")
    ax.scatter(np.arange(len(length)), optimized, marker="^", s=100, color="#666666")
    ax.set_xticks(np.arange(len(length)))
    ax.set_xticklabels(length)
    ax.set_xlabel("Loop bound (N)")
